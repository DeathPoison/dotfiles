...
proc install*( vars: DotfileModuleAttributes ): bool =
  ...
  discard installPackages( deps, packageManager = PKG_MNG, distribution = DIST )

  ## compile status bar ~ rust
  discard runCommand( "cd " & i3StatusPath & " && cargo build --release", user = USER )

  ## compile i3-gaps
  discard runCommand( "cd " & i3Path & " && autoconf --force --install", user = USER )
  ...
  discard runCommand( "cd " & i3Path & "/build && make -j8", user = USER )
